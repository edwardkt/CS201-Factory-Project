...
public class GUITruck extends GuiComponent
{
	...
	*/
	...
	enum TruckState
	{
		LOADING, LEAVING, RETURNING, ATDESTINATION
	};
...
*/			
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		else if (state == TruckState.LEAVING)
		{
			moveTruckOut();
		}
		else if( state == TruckState.ATDESTINATION && !isBroken ) {
	...
	private void moveTruckOut()
	{
		...
		{
			state = TruckState.ATDESTINATION;
			for( MyGUIGlass piece: parts ) {
				parent.getActivePieces().remove(piece.part);
				//piece.part.setVisible( false );
			}
			parts.clear();
	...
	public void eventFired(TChannel channel, TEvent event, Object[] args)
	{
		...
		else if( event == TEvent.TRUCK_BROKEN ) {
			isBroken = true;
		}
