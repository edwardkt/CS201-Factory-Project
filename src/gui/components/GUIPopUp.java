...
public class GUIPopUp extends GuiComponent implements Serializable, MouseListener
{
	...
	public void eventFired(TChannel channel, TEvent event, Object[] args)
	{
		...
		if(channel == TChannel.POPUP && event == TEvent.POPUP_DO_MOVE_UP && ((Integer)args[0]).equals( index ))
		{
			heightState = PopUpHeightState.RISING;
			return;
		}
		else if(channel == TChannel.POPUP && event == TEvent.POPUP_DO_MOVE_DOWN && ((Integer)args[0]).equals( index ))
		{
			heightState = PopUpHeightState.DROPPING;
			return;
		}
		else if(channel == TChannel.POPUP && event == TEvent.POPUP_RELEASE_GLASS && ((Integer)args[0]).equals( index ))
		{
